# Claude Code Project Instructions — chardetng-kotlin

## Project Overview

This is **chardetng-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`chardetng`](https://crates.io/crates/chardetng).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/chardetng/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/chardetng>
